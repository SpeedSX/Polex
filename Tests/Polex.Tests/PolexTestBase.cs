...
namespace Polex.Tests
{
    public abstract class PolexTestBase : AbpIntegratedTestBase
    {
        ...
        protected void UsingDbContext(Action<PolexDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<PolexDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PolexDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PolexDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PolexDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PolexDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<PolexDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PolexDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PolexDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PolexDbContext, Task<T>> func)
        {
            T result;
